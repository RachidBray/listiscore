...
* **Owner-facing widget / shortcode / block** — `[lhs_health]` (also available as a widget and a Gutenberg block) shows the score, a color-coded band, and a recommendations list sorted by potential score percentage gain, so owners know exactly what to fix first. Visible to the listing owner and site admins by default; can be set to show publicly.
